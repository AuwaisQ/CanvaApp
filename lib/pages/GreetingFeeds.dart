...
// class GreetingFeeds extends StatefulWidget {
//   String? categoryId;
//   String? displayImage;
//   GreetingFeeds({required this.categoryId, this.displayImage});
//   @override
//   _NewGreetingFeedsState createState() =>
//       _NewGreetingFeedsState(categoryId, displayImage.toString());
// }
...
class GreetingFeeds extends StatefulWidget
{
  ...
  GreetingFeeds({required this.categoryId,this.displayImage});
  @override
  _GreetingFeeds createState() => _GreetingFeeds(categoryId,displayImage.toString());
}

class _GreetingFeeds extends State<GreetingFeeds>with SingleTickerProviderStateMixin
{
  String? categoryId;
  String displayImage = "";
  ...
  _GreetingFeeds(this.categoryId, this.displayImage);
  ...
  var url = Uri.parse(webUrl + "greetingFeed");
  ...
  void getBusinessCategory() async {
    ...
    var response = await http.post(url,
        ...
        },body: jsonEncode(<String, String>
        {
          ...
          'greetingId': categoryId!,//categoryId.toString(),
          //'bcategoryId': "40",
        }));
    print(response.body);
    ...
    if (status) {
      ...
      dta = map["subposter"];
      dtaCategory=map["subgreetinglist"];
      //dtaVideo = map["videos"];
      if(displayImage=="none")
      {
        displayImage=dta[0]["image"];
      }
      dtaCategoryList.add(new NameIdBean("99999999".toString(), "All",  "99999999", true));
      for(int i=0;i<dtaCategory.length;i++)
      {
        //dta[i]["paid"]=="Free"?stts=true:stts=false;
        dtaCategoryList.add(new NameIdBean(dtaCategory[i]["subgreetingid"].toString(), dtaCategory[i]["greeting_category_name"], dtaCategory[i]["greeting_category_name"], true));
      }
  ...
  void getMyFrame() async {
    ...
    {
      //dta[i]["paid"]=="Free"?stts=true:stts=false;
      dtaFrameCategoryList.add(new NameIdBean(dtaFrameCategory[i]["id"].toString(), dtaFrameCategory[i]["frame"], dtaFrameCategory[i]["frame"], true));
    }
    // for(int i=0;i<dta.length;i++)
    // {
    //   bool stts=false;
        ...
        appBar: AppBar(
          ...
          actions: [
            ...
            haiImage?IconButton(
              ...
              icon: FaIcon(
                FontAwesomeIcons.download, color: Colors.white, size: 20,),
            ),
            // PopupMenuButton(
                ...
                Container(
                  ...
                  child: Stack(
                    ...
                    children: [
                      ...
                      temId!="none"?widgetToImage(builder: (key)
                      {
                        this.frameKey = key;
                        return Container(
                  ...
                  return Container(
                    ...
                    child: Stack(
                      ...
                      children: [
                            ...
                            children: [
                              Positioned(
                                  ...
                                  child: InkWell(
                                    ...
                                    child: InkWell(
                                      onTap: ()=>{
                                        Navigator.push(context, MaterialPageRoute(builder: (_)=>addLogoPage()))
                                      },
                                      child: Image.network(
                                        logo != null
                                            ? imageUrl + logo!
                        ...
                        isCustomFrame?Image.network(imgUrl+customFrame):temId!="none"?getCustomTemplate(temId):Stack(
                          alignment: Alignment.bottomRight,
                          children: [
                            Container(
                              height: 20,
                        ...
                        //isCustomFrame?Image.network(imgUrl + customFrame,color: _currentColor.withOpacity(0.3),fit: BoxFit.cover,):temId!="none"?getCustomTemplate(temId):lstWidget[0],
                        //ats
                        // Container(
                        //   height: 30,
                        //   width: double.infinity,
                  ...
                  SingleChildScrollView(
                    ...
                    child:
                    Container(
                      child: Row(children:List.generate(dtaCategoryList.length, (index) =>
                        ...
                        children: [
                          Container(
                            ...
                            child: GridView.builder(
                              ...
                              //     false) //posterList[index].status)
                              ,
                              itemCount: dtaFilteredPosters.isEmpty?posterList.length:dtaFilteredPosters.length
                              ,physics: ScrollPhysics(),
                            ),
                          ),
                          Container(height: 350, child:
                          videoList.isEmpty?Container(alignment: Alignment.center, child: Image.asset("assets/images/no_video.png",height: 150,width: 150,)):GridView.builder(scrollDirection: Axis.vertical,
                            shrinkWrap: true,
                              ...
                              return Stack(
                                children: [
                                  Container(
                                      ...
                                      margin: EdgeInsets.all(10),
                                      decoration: BoxDecoration(
                                          image: DecorationImage(image: NetworkImage(imageUrl+dtaVideo[index]["video_image"].toString()),fit: BoxFit.cover),
                                          color: Colors.white,
                                      ...
                                      child: InkWell(
                                        ...
                                        {
                                          displayVideo = vdoUrl +
                                              dtaVideo[index]["videopath"].toString().trim();
                                          print(displayVideo);
                          ...
                          Container(
                            ...
                            child: GridView.builder(
                              ...
                              SliverGridDelegateWithFixedCrossAxisCount(
                                  mainAxisSpacing: 5, crossAxisCount: 3),
                              itemBuilder: (context, index) =>getFrameImage(dtaFrameCategoryList[index].name,index,dtaFrameCategoryList[index].image,false)//getCatImage(posterList[index].name,index, posterList[index].image, false)
                              //     : getCatImage(searchPoster[index].name,index,searchPoster[index].image,
                              //     false) //posterList[index].status)
      ...
      child: ChoiceChip(
        ...
        onSelected: (isSelected)
        {
          ...
          if(nameIdBean.image=="99999999")
          {
            ...
            dtaFilteredPosters=posterList;//posterList.where((element) =>element.id.toLowerCase().contains(other)
          }else
          {
            dtaFilteredPosters=posterList.where((element) =>element.id.toLowerCase().contains(nameIdBean.id)).toList();
          }
  ...
  */ // String imageName=body['imageName'];
  ...
  Future<void> _renderVideo(String frameUrl, String videoUrl) async {
    ...
    //final FlutterFFprobe _f = FlutterFFprobe();
    ...
    try {
      ...
      await dio.download(
        videoUrl, "${dir?.path}/video.mp4", onReceiveProgress: (count, total) {
        print("Rec: ${count.toString()} , Total: $total");
      },).then((value) {
        log('video downloaded ${dir?.path}/video.mp4', name: 'videoStatus');
      });
      await dio.download(
        frameUrl, "${dir?.path}/frame.png", onReceiveProgress: (count, total) {
        print("Rec: ${count.toString()} , Total: $total");
      },).then((value) {
      ...
      String command = ' -i $pathOfVideo -i $pathOfFrame -filter_complex [1]scale=$heightOfVideo:$widthOfVideo[w];[0][w]overlay=5:H-h-5 -preset ultrafast -y ${dir
          ?.path}/output_${datetime.hour.toString()}.mp4';
      ...
      await FFmpegKit.execute(command).then((value) async {
        ...
        if(Platform.isIOS){
          ...
          File opFile = File('${dir?.path}/output_${datetime.hour.toString()}.mp4');
          ...
          await GallerySaver.saveVideo(opFile.path, albumName: "Download").then((value) => print("gallery video status "+value.toString()));
          await frameFile.delete();
          await videoFile.delete();
          log("frame and video file deleted",name:'fileDelete');
          pd.close();
